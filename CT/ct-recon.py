...
while x1<Nx+dN-pad and y1>-dN+pad:
    ...
    # highlight all the previous angles which have been covered
    for angle in range(0,int(a0)):
        t0 = -angle*np.pi/180
        x0 = Nx/2 + R0*np.sin(t0)
        y0 = Ny/2 - R0*np.cos(t0)
        ax.plot(x0, y0, 'o', color='r', markersize=3)
    ...
    a1 = -a0 + 90 - a_fan/2
    a2 = -a0 + 90 + a_fan/2
    wedge_det = matplotlib.patches.Wedge((x0, y0), 2*R0, a1, a2, fill=False, color=col, lw=5)
